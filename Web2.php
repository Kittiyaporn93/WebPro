...
<body>
    ...
    if(!$conn)
        ...
        mysqli_select_db($conn, $dbName) or die ("ไม่สามารถเลือกฐานข้อมูล bookstroe ได้");
    ...
    if (empty(trim($fullname)) || empty(trim($customerAddress)) || empty(trim($email)) || empty($call)) {
        ...
        exit();
    } else if (!preg_match("/^[กขฃคฅฆงจฉชซฌญฎฏฐฑฒณดตถทธนบปผฝพฟภมยรลวศษสหฬอฮะา ิ ี ั ุ ู ฺเแโใไๅๆ ำ ็ ่ ้ ๊ ์a-zA-Z ]+$/", $fullname)) {
    ...
    echo'<tr><td align="left"><a href="Web1.php">BLACK</a></td><tr>';
